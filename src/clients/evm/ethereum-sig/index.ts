...
export class EthereumSig {
  ...
  generateSalt() {
    ...
    return Number(SplitUint256.fromHex(bytesToHex([4])).toHex());
  }
  ...
  public async send(envelope: any) {
    const body = {
      method: 'POST',
      headers: {
  ...
  public async propose({
    ...
    signer: any; //Signer & TypedDataSigner;
    data: Propose;
    ...
    const message: EIP712ProposeMessage = {
      ...
      userProposalValidationParams: encodeAbiParameters(
        parseAbiParameters('(int8 index, bytes params)[]'),
        [userStrategies as any]
      ),
